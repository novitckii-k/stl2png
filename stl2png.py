...
parser.add_option("",   "--extra",      dest="extra",        default="0%",                     help="Extra space to leave around the model")
...
if not options.outputfile:
    rfile = re.sub("(?i)\.stl$", "", options.stlfiles[0])
...
for stlfile in options.stlfiles:
    tzs.load_stl_file(stlfile, transmaps[options.transform])
    
# Determin the ranges from the loaded files
tzs.set_extents(options.extra)
tzs.build_pixel_grid_structures(options.widthpixels, options.heightpixels)


def pngname(optionoutputfile, i, z):
    if re.search("(?i)%.*?d", optionoutputfile):
        return optionoutputfile % i
    if re.search("(?i)%.*?f", optionoutputfile):
...
if options.nslices != 0:
    for i in range(options.nslices):
        ...
        tzs.slice_to_png(z, pngname(options.outputfile, i, z))
...
if options.cinputs:
    ...
    while True:
        ...
        lcl = cl.split(None, 1)
        z = float(lcl[0])
        tzs.slice_to_png(z, pngname(lcl[1] if len(lcl) == 2 else options.outputfile, i, z))
        i += 1
