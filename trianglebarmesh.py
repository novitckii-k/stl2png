from basicgeo import P3, along_acc, I1
import stlgenerator
...
class TriangleBar:
    ...
    def set_fore_right_bl(self, bforeright, bar):
        if bforeright:
            self.barforeright = bar
        else:
            self.barbackleft = bar

    def get_fore_right_bl(self, bforeright):
        if bforeright:
            return self.barforeright
        else:
            return self.barbackleft
            
    def get_node_fore(self, bfore):
        if bfore:
            return self.nodefore
        else:
            return self.nodeback

    def d_get_cell_mark_right_l(self, bright):
        if bright:
            return self.cellmarkright
        else:
            return self.cellmarkleft

    def d_is_triangle_ref_bar(self):
        return self.barforeright and (self.barforeright.get_node_fore(self.nodefore == self.barforeright.nodeback).i > self.nodeback.i)
...
class TriangleBarMesh:
    def __init__(self, fname=None, trans=None, nodesortkey=lambda x: x):
        ...
        if fname is not None:
            ...
            self.build_triangle_barmesh(sr)
    ...
    def get_node_point(self, i):
        return self.nodes[i].p

    def get_bar_points(self, i):
        bar = self.bars[i]
        return bar.nodeback.p, bar.nodefore.p

    def get_tri_points(self, i):
        bar = self.bars[i]
        node2 = bar.barforeright.get_node_fore(bar.nodefore == bar.barforeright.nodeback)
    ...
    def build_triangle_barmesh(self, trpts):
        ...
        for pt, i3 in ipts:
            if not prevpt or prevpt != pt:
                self.new_node(P3(pt[0], pt[1], pt[2]))
                prevpt = pt
        ...
        for jt0, jt1, jt2 in jtrs:
            if jt0 != jt1 and jt0 != jt2 and jt1 != jt2:  # are all the points distinct?
                ...
                tbars[-3].set_fore_right_bl(jt0 < jt1, tbars[-2])
                tbars[-2].set_fore_right_bl(jt1 < jt2, tbars[-1])
                tbars[-1].set_fore_right_bl(jt2 < jt0, tbars[-3])
        del jtrs
        ...
        for bar in tbars:
            if prevbar and prevbar.nodeback == bar.nodeback and prevbar.nodefore == bar.nodefore and \
                ...
                node2 = bar.barforeright.get_node_fore(bar.nodefore == bar.barforeright.nodeback)
                bar2 = bar.barforeright.get_fore_right_bl(bar.nodefore == bar.barforeright.nodeback)
                assert bar2.get_node_fore(bar2.nodeback == node2) == bar.nodeback
                assert bar2.get_fore_right_bl(bar2.nodeback == node2) == bar
                bar2.set_fore_right_bl(bar2.nodeback == node2, prevbar)
                prevbar = None
            else:
                prevbar = bar
                assert prevbar.i == -1
    ...
    def get_bar_mesh_triangles(self):
        tris = []
        for bar in self.bars:
            if bar.barforeright:
                ...
                if node2.i > bar.nodeback.i:
                    node0 = bar.nodeback
                    node1 = bar.nodefore
                    tris.append((node0.p.x, node0.p.y, node0.p.z, node1.p.x, node1.p.y, node1.p.z, node2.p.x, node2.p.y,
                                 node2.p.z))
        return tris
